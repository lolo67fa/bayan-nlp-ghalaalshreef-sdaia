...
- **الحالة:** **`ADOPT_FOR_EXPERIMENT`** — تحسّن مقاس، لكن لا يُخدَم بعد- **Commit SHA:** `3c22f9d`
...
| mrr@3 قبل إعادة الترتيب `[MEASURED_SMOKE]` | 0.6667 | `reports/retrieval_metrics.json` |
| mrr@3 بعد إعادة الترتيب `[MEASURED_SMOKE]` | **0.7222** — تحسّن **+0.0556** | نفسه |
| زمن إعادة الترتيب | median 15.70 ms · p95 17.16 ms، الإحماء مستبعد | نفسه |
| القرار | `ADOPT_FOR_EXPERIMENT` | نفسه |
**سبب الاعتماد للتجربة فقط:** التحسّن مقاس (+0.0556 في mrr@3) ومتسق مع التشخيص: recall كان 1.0 أصلًا، فالضعف في الترتيب لا في الاسترجاع — وهذا بالضبط ما تعالجه إعادة الترتيب. لكن ستة استعلامات لا تفصل المكسب عن ضجيج إعادة العيّنة، ولم تُحسب فترة ثقة، والمسار المخدوم بلا ميزانية زمن لمرحلة ثانية.

**شرط الاعتماد الكامل:** تحسّن مقاس على البيانات المجمّدة بفترة ثقة لا تشمل الصفر، وزمن p95 مضاف يسع الميزانية.
